...
if(isset($_POST['email']) && isset($_POST['lastname']) && isset($_POST['username']) ) {
    ...
    if(htmlentities($email) && strip_tags($username) && strip_tags($fulladdress)) {
        ...
        $insert = $con -> prepare("INSERT INTO users (trackingId,email,username,firstname,lastname,userCategory,password,profilepics,rating,emailCode,accountStatus,ip,fullAddress,state,city,zipcode,totalsubAmount,subscriptionStatus,timestatus) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        ...
        if($insert -> execute()) {
            ...
            $emailUname = $firstname;
            $_SESSION['IUname'] = $username;
